...
FIXME:
- Processing human-legible 
- ULAN searching.
- Preview/suggest function to streamline picking through multiple term entries. 
"""

# FIXME:
# - Flow for processing arbitrary human-legible material descriptions:
#	- Import NKLT
#	- Split material by and, or, etc, remove punctuation and (anything in parens)
#	- Store tagged material components with part of speech IDs in a separate array
#	- Search SPARQL for string component, if nothing found strip adjectives/adverbs/prepostions and retry terms
...
metadata = {
	...
	"view": { "url" : "http://vocab.getty.edu/aat/{{id}}"},
...
def ulan_search(query):
	...
	ulan_query_head = 'select * {?artists luc:term "'
	ulan_query_tail = '"skos:inScheme ulan:; rdf:type gvp:PersonConcept'
	ulan_sparql_query = ulan_query_head+query.lower()+ulan_query_tail

	# send requests
	ulan_query_response = requests.get(url_base, params={'query': ulan_sparql_query}) # should be try/catch for HTTP errs
...
def search(query):
	...
	response_json_nohead = response_json['results']['bindings'] # strip header of 'results' and 'bindings'
